<?php	
	...
	$asli->allow_rolls = ['kassir','sotuv','saqlash'];
	...
	if(isset($_GET['id']) or isset($_GET['bulim_id'])){
		...
		$jamibank = 0;
		$jamikarta = 0;
		if(isset($_GET['harajat_category_id']) AND isset($_GET['sana1']) AND isset($_GET['sana2'])){
			$sana1 = strtotime($_GET['sana1']);
			$sana2 = strtotime($_GET['sana2']);
			$hid = $_GET['harajat_category_id'];
			if($hid>0){
				$harajatlar = $asli->getdatas('harajat',[],"category_id='$hid' AND sana>='$sana1' AND sana<'$sana2'");
			}
			else{
				$harajatlar = $asli->getdatas('harajat',[],"sana>='$sana1' AND sana<'$sana2'");	
			}
		}
		else{
		...
		foreach ($harajatlar as $key => $harajat) {
			$user = $asli->getdata('user',['id'=>$harajat['user_id']]);
			$cat = $asli->getdata('harajat_category',['id'=>$harajat['category_id']]);
			$ret[$i]['id'] = $harajat['id'];
			$ret[$i]['naqdsum'] = $harajat['naqdsum'];
			$ret[$i]['naqdusd'] = $harajat['naqdusd'];
			$ret[$i]['bank'] = $harajat['bank'];
