<?php	
	...
	if(isset($_GET['id'])){
		...
		if(isset($_GET['sana1']) && isset($_GET['sana2'])){
			...
			$sana1 = date("d.m.Y 00:00:00",time());
			$sana1 = date("d.m.Y 00:00:00",time()+86400);
		...
		foreach ($orders as $key => $order) {
			...
			$ret[$i]['summa'] = $order['summa'];
			$ret[$i]['naqd'] = $order['naqd'];
			$ret[$i]['naqdusd'] = $order['naqdusd'];
			$ret[$i]['plastik'] = $order['plastik'];
			$ret[$i]['karta'] = $order['karta'];
			$ret[$i]['valyuta'] = $order['valyuta'];
		...
		if($i==0){
			$asli->resp['message'] = "Bugun savdo bo'lmagan";
		}
		$asli->resp['data'] = $ret;
	}
	$asli->print_json()
