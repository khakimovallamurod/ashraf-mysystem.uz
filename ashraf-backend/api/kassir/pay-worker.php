<?php	
	...
	if(isset($data->naqdsum) && strlen($data->izoh)>=1){
		...
		if($data->naqdusd>0){
			$summa += $data->naqdusd * $data->valyuta;
		}
		if(!$sql){
			$asli->kalit = 0;
		}
		$sql = $asli->insert('worker_pay_history',[
			'summa' => $summa,
		...
		$worker = $asli->getdata('workers',['id'=>$data->worker_id]);
		$sql = $asli->update('workers',[
			'balans' => $worker['balans'] - $summa
		],['id'=>$worker['id']]);
		$sms = $asli->sendsms($worker['telefon'],"Oylik avans berildi : ".$summa);
		if(!$sql){
			$asli->kalit = 0;
		}
		if($asli->kalit == 1){
