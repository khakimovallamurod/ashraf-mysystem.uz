<?php	
	...
	if(isset($_GET['id'])){
		...
		if($method=="DELETE"){
			$sql = $asli->delete('dvident_category',['id'=>$_GET['id']]);
			if($sql){
		...
		if($method=="GET"){
			...
			$hcategory = $asli->getdata('dvident_category',['id'=>$_GET['id']]);
			$asli->resp['data'] = $hcategory;
		}
	}
	else{
		$asli->resp += ['success'=> true, 'message' => "Muvaffiqqiyatli"];
		$hcategory = $asli->getdatas('dvident_category',[],"1");		
		$ret = [];
		$i = 0;
