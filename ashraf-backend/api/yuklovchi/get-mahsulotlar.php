<?php	
	...
	if(isset($_GET['id']) || isset($_GET['bulim_id'])){
		...
		foreach ($polkas as $key => $polka) {
			...
			$j = 0;
			$p_id = $polka['id'];
			$sbhistorys = $asli->getdatas('putpolka',['polka_id'=>$polka['id']],"polka_id='$p_id' AND massa>'0'");
			foreach ($sbhistorys as $key2 => $sbhistory) {
				if(round($sbhistory['massa'],2)==0){
					continue;
				}
				$product = $asli->getdata('products',['id'=>$sbhistory['product_id']]);
				$temp[$j]['id'] = $sbhistory['id'];
				$temp[$j]['polka_id'] = $p_id;
				...
				$temp[$j]['massa'] = round($sbhistory['massa'],2);
				$j++;
			}
