<?php	
	...
	if(isset($_GET['id'])){
		...
		$zayavkalar = $asli->getdatas('qaytamaydalash_items',['id'=>$_GET['id']]);
		foreach ($zayavkalar as $key => $zayavka) {
			$maydalovchi = $asli->getdata('user',['id'=>$zayavka['yuklovchi_id']]);
			...
			$ret['maydalovchi'] = $maydalovchi['familya']." ".$maydalovchi['ism'];
		...
		$asli->resp += ['success'=> true, 'message' => "Muvaffiqqiyatli"];
		$i = 0;
		$zayavkalar = $asli->getdatas('qaytamaydalash_items',['status'=>'joylanmoqda']);
		foreach ($zayavkalar as $key => $zayavka) {
			$maydalovchi = $asli->getdata('user',['id'=>$zayavka['maydalovchi_id']]);
			$product = $asli->getdata('products',['id'=>$zayavka['product_id']]);
